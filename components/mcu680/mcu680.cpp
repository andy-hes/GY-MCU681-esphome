...
void MCU680Component::setup() {
  ...
  ESP_LOGCONFIG(TAG, "MCU680 / MCU681 initialized");
}
...
void MCU680Component::parse_frame_(const uint8_t *frame) {
  ...
  // [9]  pressure mid
  // [10] pressure low
  // [11] IAQ accuracy high nibble, IAQ high nibble low
  // [12] IAQ low byte
  // [13] gas resistance byte 1
  // [14] gas resistance byte 2
  // [15] gas resistance byte 3
  // [16] gas resistance byte 4
  // [17] altitude high
  // [18] altitude low
  // [19] checksum
  ...
  const uint32_t gas_raw = (uint32_t(frame[13]) << 24) |
  ...
  ESP_LOGD(TAG,
           "Frame ok: temp=%.2fC hum=%.2f%% press=%.2fhPa iaq_acc=%u iaq=%u gas=%.1fkOhm alt=%dm",
